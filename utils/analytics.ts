// Google Analytics 4 Configuration and Utilities for Nuxt 4 SSR
...
// Helper to get measurement ID (only in browser context)
function getMeasurementId(): string {
  if (!isBrowser) return GA_MEASUREMENT_ID;

  try {
    const config = useRuntimeConfig();
    return (config.public.gaMeasurementId as string) || GA_MEASUREMENT_ID;
  } catch {
...
export function initGA(): void {
  ...
  // Skip GA in development if preferred
  if (isDevelopment) {
